...
public class ReactiveActiveMqConsumer implements ActiveMqConsumerPort {
    ...
    public record MessageRecord(String destination, String message, String correlationId, Map<String,String> headers, String traceparent) {}

    private final Sinks.Many<MessageRecord> messageSink = Sinks.many().multicast().onBackpressureBuffer();
    private final JmsTemplate jmsTemplate;

    @Value("${activemq.dlq.destination:ActiveMQ.DLQ}")
    private String dlqDestination;

    public ReactiveActiveMqConsumer(JmsTemplate jmsTemplate) { this.jmsTemplate = jmsTemplate; }
    ...
    public void handleMessage(Message raw) {
        try {
            String destination = raw.getJMSDestination() != null ? raw.getJMSDestination().toString() : "unknown";
            ...
            while (names != null && names.hasMoreElements()) {
                ...
                if (n != null) {
                    ...
                    try { headers.put(key, String.valueOf(raw.getObjectProperty(key))); } catch (Exception ignore) {}
                }
            }
            ...
            if (traceParent == null) {
                traceParent = com.resilient.messaging.TracingHeaderUtil.ensureTracing(headers).get("traceparent");
            }
            ...
            try {
                final String cid;
                try { cid = raw.getJMSCorrelationID(); } catch (Exception ignore) { throw new IllegalStateException("No correlation id", ignore); }
                ...
                jmsTemplate.send(dlqDestination, session -> {
                    ...
                    msg.setStringProperty("originalDestination", raw.getJMSDestination()!=null?raw.getJMSDestination().toString():"unknown");
